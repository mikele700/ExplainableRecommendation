	...
	for fold in range(numfolds):
		...
		for u, i, r in zip(test.u, test.i, test.r):
			...
			if (user in entities) and (item in entities):
                ...
                # Get explaination
				top_3_items, top_3_item_similarities, friends_liked, top_3_users, top_3_user_similarities = explain(Dr, Dp, user, item, args.min_u_id, args.max_u_id)
...
def explain(Dr, Dp, target_user, target_item, min_u_id, max_u_id):
    ...
    if len(items) > 0:
        for i in range(3):
            ...
            top_3_item_similarities.append(max_similarity)
            items.pop(max_index)
    ...
    if len(users) > 0:     
        for i in range(3):
            ...
            top_3_user_similarities.append(max_similarity)
            users.pop(max_index)
            similarity.pop(max_index)
            if len(users) == 0:
                break
    ...
    return top_3_items, top_3_item_similarities, friends_liked, top_3_users, top_3_user_similarities
